...
public class parent_from_child {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Parents of each node are: ");
        for(var p:mp.entrySet()){
            System.out.println(p.getKey().x+" -> "+p.getValue().x);
        }
        System.out.println();
        int nn=sc.nextInt();
        System.out.println("Enter the starting node to be burned : "+nn);
        List<List<Integer>> lst= burntree(root,nn);
        System.out.println("Time taken to burn the tree from  "+nn+" : "+(lst.size()-1));
        System.out.println("Burned Nodes are: ");
        for(var i:lst)
...
static List<List<Integer>> burntree(Tree root,int data){
...
static void find_Parents(Tree root,Map<Tree,Tree> mp){
    if(root==null)return ;
    Queue<Tree> q=new LinkedList<>();
    q.add(root);
