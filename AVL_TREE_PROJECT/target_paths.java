...
public class target_paths {
    public static void main(String[] args) {
        ...
        System.out.println();
        List<Integer> path=getPath(root,trgt);
        System.out.println("Target : "+trgt);
        System.out.println();
        System.out.println("Target Paths:");
        System.out.println(path);
        
    }


static List<Integer> getPath(tree root,int data){
    List<Integer>l=new ArrayList<>();
    if(root==null)return l;
    l.add(root.x);
    boolean get=helper(root,data,l);
    if(get)return l;
    return l;
}
static boolean helper(tree root, int data,List<Integer> l){
    if(root.x==data)return true;
    if(root.left!=null){
        l.add(root.left.x);
        boolean found=helper(root.left,data,l);
        if(found)return true;
        l.remove(l.size()-1);
    }
    if(root.right!=null){
        l.add(root.right.x);
        boolean found=helper(root.right,data,l);
        if(found)return true;
        l.remove(l.size()-1);
    }
    return false;
